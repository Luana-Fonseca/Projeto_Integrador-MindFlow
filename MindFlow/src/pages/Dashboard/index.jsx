...
function Dashboard({ navigateTo }) {
    ...
    const handleSaveSprint = async (sprintData) => {
        ...
        // 🎯 NOVO: Lógica de Checagem POST vs. PUT
        const isEditing = sprintData.id && String(sprintData.id).startsWith('sprint-');
        const method = isEditing ? 'PUT' : 'POST';

        // Se for edição, a URL deve incluir APENAS o ID numérico, sem o prefixo 'sprint-'.
        const rawSprintId = isEditing ? sprintData.id.replace('sprint-', '') : '';

        const url = isEditing
            ? `${API_BASE_URL}/sprints/${rawSprintId}`
        ...
        const dataToSend = {
            ...
            usuarioId: usuarioId
            // NUNCA envie o 'sprint-ID' do frontend para o backend, apenas os dados e a ID na URL
        };

        try {
            console.log(`🔄 Enviando ${method} para ${url} com dados:`, dataToSend);
            ...
            if (response.ok) {
                // Pega o ID REAL, seja ele da resposta (POST) ou o ID de entrada (PUT)
                let savedSprintId = sprintData.id;
                let savedSprintData = dataToSend;

                if (method === 'POST') {
                    const responseData = await response.json();
                    savedSprintId = `sprint-${responseData.id}`; // Formata para o padrão do React
                    savedSprintData = { ...dataToSend, id: savedSprintId };
                } else {
                    // Se for PUT, a resposta pode não ter JSON, mas o ID já existe
                    savedSprintData = { ...dataToSend, id: sprintData.id };
                }

                // Atualiza o estado local
                setSprints(prev => ({
                    ...prev,
                    [savedSprintId]: savedSprintData
                }));

                // Recarrega tarefas para atualizar associações, caso tenha havido edição de dados.
                carregarTarefasDoBanco();

                handleCloseSprintModal();
                console.log(`✅ Sprint ${method === 'PUT' ? 'atualizada' : 'criada'} com sucesso: ${savedSprintId}`);
            } else {
                console.error(`❌ Erro HTTP ao salvar sprint (${response.status}):`, await response.text());
                alert(`Erro ao ${isEditing ? 'atualizar' : 'salvar'} sprint no servidor.`);
            }
        } catch (error) {
